...
* keyword类型会直接对字符串建立倒排索引，text类型会对字符串进行分词再建立倒排索引
...
* get shop_dev_shop_order_comment/_mapping

* 7.0后直接使用index和document两层，没有type了
...
* query下可选的搜索关键字
  ...
  * match：模糊匹配，只要出现匹配的一个词就能匹配上
  * match_phrase：精确的匹配，一定要出现这个词

* highlight：高亮搜索词，常用语全文搜索中的关键词搜索。
...
* 概念：聚合是一种可以在数据上生产复杂的分析统计，类似groupby。
...
* 倒排索引就是将所要索引的关键字，在查询的字段中先进行分词，然后按匹配度进行筛选。
